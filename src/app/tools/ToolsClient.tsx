  ...
  return (
    <main className="bg-[#fbfbf9] text-[#0b0f19] min-h-screen antialiased">
      <div className="mx-auto w-full max-w-3xl lg:max-w-6xl px-5 sm:px-8">
        ...
        {/* masthead */}
        <header className="pt-7 sm:pt-10 pb-5">
          ...
          <h1 className="mt-2.5 font-display text-[clamp(26px,5vw,37px)] font-extrabold leading-[1.05] tracking-[-0.028em]">
            The tool index<span className="text-emerald-600">.</span>
          </h1>
          <p className="mt-2.5 max-w-xl text-[15px] sm:text-[16px] leading-relaxed text-slate-500">
        ...
        <div className="sticky top-[56px] z-20 -mx-5 sm:-mx-8 bg-[#fbfbf9]/95 backdrop-blur px-5 sm:px-8 pt-2.5 pb-2.5 border-b border-[#0b0f19]/[0.07] shadow-[0_6px_16px_-12px_rgba(11,15,25,0.1)]">
          <div className="flex items-center gap-2.5 rounded-xl border border-[#0b0f19]/10 bg-white px-4 py-2.5 shadow-[0_1px_2px_rgba(11,15,25,0.04)] transition-[border-color,box-shadow] focus-within:border-emerald-500 focus-within:shadow-[0_0_0_4px_rgba(4,120,87,0.08)]">
            ...
            <input
              ...
              className="w-full min-w-0 border-none bg-transparent text-[16.5px] font-medium placeholder:text-slate-400 focus:outline-none focus:ring-0"
            />
            {query ? (
              ...
              <kbd className="hidden sm:block flex-none font-mono text-[11px] text-slate-400 border border-slate-200 bg-slate-50 rounded px-1.5 py-0.5">⌘K</kbd>
            )}
          </div>

          <div className="mt-2.5 flex gap-1.5 overflow-x-auto no-scrollbar lg:flex-wrap lg:overflow-visible">
            {CATEGORIES.map((c) => {
              ...
              return (
                <button
                  ...
                  className="flex-none rounded-lg bg-white border border-[#0b0f19]/[0.09] px-3 py-1.5 text-[12.5px] font-bold whitespace-nowrap text-slate-600 shadow-[0_1px_2px_rgba(11,15,25,0.03)] transition-all hover:border-emerald-600/40 hover:text-emerald-800 hover:shadow-[0_2px_8px_-3px_rgba(4,120,87,0.25)] hover:-translate-y-px active:scale-95"
                >
        ...
        {/* result meta */}
        <p className="pt-3.5 pb-0.5 font-mono text-[12px] text-slate-400 tabular-nums">
        ...
        {sections.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <button onClick={() => setQuery('')} className="mt-2 text-[15.5px] font-bold text-emerald-700 underline">
            ...
            {sections.map(({ cat, items }) => {
              ...
              return (
                <section
                  key={cat.id}
                  id={`sec-${cat.id}`}
                  className="mt-4 overflow-hidden rounded-2xl border border-[#0b0f19]/[0.07] bg-white shadow-[0_1px_2px_rgba(11,15,25,0.03)] transition-shadow duration-200 hover:shadow-[0_10px_30px_-18px_rgba(11,15,25,0.35)]"
                >
                  {/* tinted header band in the category's own colour */}
                  <div
                    className="flex items-center gap-3 border-b border-[#0b0f19]/[0.05] px-4 py-3"
                    style={{ background: `linear-gradient(90deg, ${cat.color}12 0%, transparent 65%)` }}
                  >
                    <span
                      className="flex h-8 w-8 flex-none items-center justify-center rounded-lg shadow-[0_2px_8px_-3px_rgba(11,15,25,0.3)]"
                      style={{ background: cat.color, color: '#fff' }}
                    >
                      <Icon className="h-4 w-4" />
                    </span>
                    <h2 className="font-display text-[16px] font-bold tracking-[-0.01em]">{cat.label}</h2>
                    <span className="font-mono text-[11.5px] text-slate-400 tabular-nums">
                      {items.filter((i) => i.status === 'live').length}
                    </span>
                    <Link
                      href={`/category/${cat.id}`}
                      className="ml-auto text-[12.5px] font-bold text-emerald-700 hover:underline whitespace-nowrap"
                    >
                      Area guide →
                    </Link>
                  </div>
                    ...
                    {items.map((t) =>
                      t.status === 'live' ? (
                        <li key={t.href + t.label} className="break-inside-avoid">
                          <Link
                            href={t.href}
                            className="group relative flex items-center gap-2.5 rounded-lg px-2.5 py-[7px] transition-colors hover:bg-slate-50 active:bg-emerald-50/60"
                          >
                            <span className="absolute left-0 top-1/2 h-0 w-[2.5px] -translate-y-1/2 rounded-full bg-emerald-500 transition-all duration-200 group-hover:h-[55%]" />
                            <span className="min-w-0 flex-1 flex flex-wrap items-baseline gap-x-2">
                              <span className="text-[15px] font-semibold leading-snug group-hover:text-emerald-800">
                                {t.label}
                              </span>
                              <span className="truncate text-[12.5px] text-slate-400">{t.hint}</span>
                            </span>
                            <ArrowUpRight className="h-4 w-4 flex-none text-slate-300 transition-all group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-emerald-700" />
                          </Link>
                        </li>
                      ) : (
                        <li key={t.href + t.label} className="break-inside-avoid">
                          <div className="flex items-center gap-2.5 px-2.5 py-[7px] select-none">
                            <span className="min-w-0 flex-1 flex flex-wrap items-baseline gap-x-2">
